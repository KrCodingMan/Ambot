...
java.lang.Thread.sleep(1000); // 대기 
...
switch(trimMsg)
{
   ...
   case "보":
   case "뷰":
      replier.reply(randompicker(["지","빨"]));
      break;
      
}if(msg.indexOf("앰봇예아니오로대답해줘 ") ==0 && msg.includes("?"))
{
    var randNum = Math.floor(Math.random()*2);
    if(randNum ==0)
       replier.reply(randompicker(["예","ㅇㅇ","그건 ㅇㅇ","ㅇ", "Yes", "그걸 왜 물어봄\n 당연히 Yes지","O"])); 
    else 
      replier.reply(randompicker(["아니요","ㄴㄴ","그건 ㄴㄴ","ㄴ", "No", "그걸 왜 물어봄\n 당연히 No지","X"])); 
}
if(["/심심","/나심심해","/심심해","심심해","심심"].includes(msg.trim()))
{
    replier.reply(randompicker(urllist));
}
else if(["/아재개그","/아재개그해줘"].includes(msg.trim()))
{
   var idx = Math.floor(Math.random()* Object.keys(ajae).length);
   //replier.reply(idx);
   replier.reply(Object.keys(ajae)[idx]);
   java.lang.Thread.sleep(7000);
   replier.reply(ajae[Object.keys(ajae)[idx]]+ "ㅋ");
}
...
if(msg.indexOf("/아이템목록") ==0){
   
   var filelist = new java.io.File(FileStream.getSdcardPath()+"/Ambot/TestGame/Item/").listFiles();

   var itemlist = "";
   for(i in filelist){
      var itemname = "";
      itemname = filelist[i]+"";
      itemname = itemname.replace(FileStream.getSdcardPath()+"/Ambot/TestGame/Item/","");
