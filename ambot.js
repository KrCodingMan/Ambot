...
{
   var idx = Math.floor(Math.random()* ajae.length);
   replier.reply(ajae.idx.que);
   java.lang.Thread.sleep(7000);
   replier.reply(ajae.idx.ans + " ㅋ");
}
...
var ajae = {
   1: { que: "왕이 넘어지면?", ans: "킹콩" },
   2: { que: "공이 웃으면? ", ans: "풋볼" },
   3: { que: "인천앞바다의 반댓말은?", ans: "인천 엄마다" },
   4: { que: "오리가 얼면?", ans: "언덕" },
   5: { que: "칼이 정색하면?", ans: "검정색" },
   6: { que: "자가용의 반댓말은?", ans: "커용" },
   7: { que: "흑인이 울면?", ans: "흑흑" },
   8: { que: "흑인이 빡치면?", ans: "깜빡" },
   9: { que: "흑인이 정색하면?", ans: "검정색" },
   10: { que: "흑인이 싸면?", ans: "깜찍" },
   11: { que: "흑인이 실종되면?", ans: "깜깜무소식" },
   12: { que: "흑인이 놀라면?", ans: "깜짝" }
}